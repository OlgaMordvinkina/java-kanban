...
public class KVServer {
    ...
    public KVServer() throws IOException {
        ...
        server.createContext("/loadAll", this::loadAll);
    }

    public KVServer(String apiToken, HttpServer server) {
        ...
        try {
            ...
            if ("GET".equals(httpExchange.getRequestMethod())) {
                ...
                if (data.get(key) == null) {
                    System.out.println("Данные для ключа '" + key + "', данные отсутствуют");
    ...
    private void loadAll(HttpExchange httpExchange) {
        try {
            System.out.println("\n/loadAll");
            if (!hasAuth(httpExchange)) {
            ...
            if ("GET".equals(httpExchange.getRequestMethod())) {
                sendText(httpExchange, gson.toJson(data.values()));
                System.out.println("Значение успешно отправлено в ответ на запрос!");
                httpExchange.sendResponseHeaders(200, 0);
            } else {
                System.out.println("/load ждет GET-запрос, а получил: " + httpExchange.getRequestMethod());
                httpExchange.sendResponseHeaders(405, 0);
